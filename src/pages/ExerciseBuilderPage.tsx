import { ArrowDown, ArrowUp, FolderOpen, ListPlus, RotateCcw, Save, Trash2, UserPlus } from 'lucide-react'
...
export function ExerciseBuilderPage() {
  ...
  const addPathologyTemplate = (template: (typeof templates)[number], pathologyLabel: string) => {
    ...
    setPathologySelection((current) => [...current, { templateId: template.id, name: template.name, config: { ...template.config } }])
    setNotice(`“${template.name}” se agregó desde ${pathologyLabel}. La selección ahora tiene ${pathologySelection.length + 1} ejercicios.`)
  }
  ...
  return (
    <div className="space-y-7">
      ...
      <PathologyRecommendations templates={templates} selectedTemplateIds={pathologySelection.map((item) => item.templateId)} onLoadTemplate={(template, pathology) => addPathologyTemplate(template, pathology.label)}/>
