...
import { completeSessionAssignment, completeSupervisedInPersonSession, createSessionAssignment, createTreatmentCycle, deleteSessionAssignment, duplicateInPersonAssignmentAsHome, getCurrentPatientAssignment, listProfessionalAssignments, listSessionAssignments, listTreatmentCycles, startSessionAssignment, startSupervisedInPersonSession, updateSessionAssignment, type SessionAssignmentRecord, type SessionCompletionInput, type SupervisedSessionCompletionInput } from './repository'
...
export function useDeleteSessionAssignment(patientId:string){const client=useQueryClient();return useMutation({mutationFn:(assignment:SessionAssignmentRecord)=>deleteSessionAssignment(assignment),onSuccess:async()=>{await client.invalidateQueries({queryKey:sessionKeys.assignments(patientId)});await client.invalidateQueries({queryKey:sessionKeys.all})}})}
