...
export function validateSession(values: SessionFormValues) {
  ...
  const firstQuestIndex = values.exercises.findIndex((exercise) => exercise.displayMode === 'quest_browser')
  const hasNonQuestAfterQuest = firstQuestIndex >= 0 && values.exercises.slice(firstQuestIndex + 1).some((exercise) => exercise.displayMode !== 'quest_browser')
  const hasVrBoxAndQuest = questExercises.length > 0 && values.exercises.some((exercise) => exercise.displayMode === 'vr_box')

  const immersiveExercises = values.exercises.filter((exercise) => exercise.purpose === 'immersive_context')
  const nonQuestImmersive = immersiveExercises.filter((exercise) => exercise.displayMode !== 'quest_browser')
  ...
  if (nonQuestImmersive.length > 0 && immersiveExercises.length !== values.exercises.length) setExerciseError('Un bloque contextual de Cardboard no se mezcla con RVO, tareas físicas ni ejercicios cognitivos dentro de la misma sesión.')
  ...
  if (hasVrBoxAndQuest) setExerciseError('Una sesión mixta puede combinar PC y Quest, pero no agrega VR Box o Cardboard como tercer dispositivo.')
  if (hasNonQuestAfterQuest) setExerciseError('En una sesión mixta, realizá primero los ejercicios sin Quest y dejá un único bloque Quest al final.')
  ...
  const contextualQuest = questExercises.filter((exercise) => exercise.purpose === 'immersive_context')
  if (contextualQuest.length > 0 && contextualQuest.length !== questExercises.length) setExerciseError('El bloque Quest contextual 360° debe ser homogéneo: no mezcles sus escenarios con paneles 2D o patrones procedurales en el mismo bloque.')
  ...
  if (proceduralImmersive.length > 0 && proceduralImmersive.length !== questExercises.length) setExerciseError('El bloque Quest procedural debe contener únicamente ejercicios WebXR procedurales. Las tareas sin Quest pueden ubicarse antes del bloque.')
