        ...
        <section className="rounded-2xl border border-[#E9E7E7] bg-white p-5">
          ...
          {config.displayMode === 'quest_browser' && !isImmersive && <div className="mt-4 rounded-2xl border border-[#B9D9C5] bg-[#F6FBF8] p-4">
            ...
            {questProceduralImmersive && <details className="group mt-4 rounded-xl border border-[#B9D9C5] bg-white p-4">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="text-[11px] font-black text-[#2F2F2F]">Geometría<select disabled={!isFree} className={`${input} disabled:bg-[#F7F6F4]`} value={isFree ? config.questImmersiveGeometry : recommendedQuestGeometry(config)} onChange={(event) => set('questImmersiveGeometry', event.target.value as QuestImmersiveGeometry)}><option value="curved_panel">Panel curvo frontal</option><option value="cylinder">Cilindro envolvente</option><option value="front_disc">Disco frontal</option><option value="particle_tunnel">Túnel de puntos 3D</option></select><span className="mt-1 block font-normal text-[#747474]">{isFree ? 'Libre permite elegirla; revisá que sea coherente con el estímulo.' : 'La plataforma la selecciona según el patrón y la finalidad.'}</span></label>
