...
function CompatibleExercisePlayer({ config, onExit, onSkip, onComplete, preparationSeconds, fullscreenTargetRef }: ExercisePlayerProps) {
  ...
  return <div ref={containerRef} className="fixed inset-0 z-[100] overflow-hidden bg-[#081113] text-white" onPointerMove={showControls} onPointerDown={showControls} onKeyDown={handleKeyDown} role="application" aria-label={`Reproductor: ${config.name}`} tabIndex={-1}>
    ...
    {!vrBox && !questImmersive && paused && <div className="pointer-events-none absolute inset-0 z-40 grid place-items-center bg-black/38 p-5"><div className="max-w-xl rounded-2xl bg-black/72 px-5 py-4 text-center backdrop-blur"><p className="text-sm font-black">{preparing ? 'Preparación en pausa' : 'Ejercicio en pausa'}</p>{config.stopCriteria && <p className="mt-3 text-xs font-bold leading-5 text-[#ffb8bd]"><strong>Criterio de detención:</strong> {config.stopCriteria}</p>}</div></div>}
