-- Permite una única transferencia clínica PC -> Quest dentro de la misma sesión.
-- El visor recibe solamente el sufijo Quest; la historia clínica permanece en la
-- pantalla profesional y el resultado se combina antes del cierre supervisado.
...
begin
  ...
  if jsonb_typeof(coalesce(target_plan_definition -> 'exercises', '[]'::jsonb)) <> 'array'
     or jsonb_array_length(coalesce(target_plan_definition -> 'exercises', '[]'::jsonb)) = 0 then
    raise exception 'La sesión no contiene ejercicios disponibles para Quest.' using errcode = '22023';
  end if;

  select count(*)::integer
  into quest_exercise_count
  ...
  if quest_exercise_count = 0 then
    raise exception 'La sesión no contiene ejercicios disponibles para Quest.' using errcode = '22023';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(coalesce(target_plan_definition -> 'exercises', '[]'::jsonb)) with ordinality current_exercise(exercise, position)
    where coalesce(current_exercise.exercise ->> 'displayMode', 'standard') <> 'quest_browser'
      and exists (
        select 1
        from jsonb_array_elements(coalesce(target_plan_definition -> 'exercises', '[]'::jsonb)) with ordinality previous_exercise(exercise, position)
        where previous_exercise.position < current_exercise.position
          and coalesce(previous_exercise.exercise ->> 'displayMode', 'standard') = 'quest_browser'
      )
  ) then
    raise exception 'Los ejercicios Quest deben formar un único bloque al final de la sesión.' using errcode = '22023';
  end if;
  ...
  loop
    ...
    begin
      insert into public.quest_session_pairings (
        assignment_id, professional_id, code_hash, status, expires_at
      ) values (
        target_assignment_id, auth.uid(), encode(extensions.digest(raw_code, 'sha256'), 'hex'), 'ready', pairing_expires_at
      ) returning id into pairing_id;
      exit;
    exception when unique_violation then
      -- Reintenta si otro vínculo listo ya usa el mismo código efímero.
    end;
  end loop;

  insert into public.audit_events (actor_user_id, action, entity_type, entity_id, metadata)
  values (
    auth.uid(), 'quest_session_pairing_created', 'session_assignment', target_assignment_id,
    jsonb_build_object('pairing_id', pairing_id, 'expires_at', pairing_expires_at, 'quest_exercise_count', quest_exercise_count)
  );
...
begin
  ...
  select coalesce(jsonb_agg(source.exercise order by source.position), '[]'::jsonb)
  into quest_exercises
  from jsonb_array_elements(coalesce(target_definition -> 'exercises', '[]'::jsonb)) with ordinality source(exercise, position)
  where coalesce(source.exercise ->> 'displayMode', 'standard') = 'quest_browser';

  if jsonb_array_length(quest_exercises) = 0 then
    update public.quest_session_pairings set status = 'revoked', updated_at = now() where id = pairing_row.id;
    raise exception 'La sesión ya no contiene un bloque Quest válido.' using errcode = '22023';
  end if;
  ...
  insert into public.audit_events (actor_user_id, action, entity_type, entity_id, metadata)
  values (
    null, 'quest_session_pairing_claimed', 'session_assignment', pairing_row.assignment_id,
    jsonb_build_object('pairing_id', pairing_row.id, 'professional_id', pairing_row.professional_id, 'quest_exercise_count', jsonb_array_length(quest_exercises))
  );
...
comment on function public.create_quest_session_pairing(uuid)
is 'Crea un vínculo efímero para el único bloque Quest situado al final de una sesión presencial.';

comment on function public.claim_quest_session_pairing(text)
is 'Entrega al visor solamente los ejercicios Quest; no expone el bloque previo ejecutado en la PC.';
