...
Una sesión presencial puede combinar tareas de Pantalla 2D y Quest con una sola transferencia: primero se completa el bloque sin Quest en la PC y luego un único bloque Quest al final. No se admite volver a la PC ni agregar VR Box/Cardboard como tercer dispositivo. La estación recibe solamente el bloque Quest y el cierre profesional suma tiempos, omisiones y eventos de ambos bloques. Dentro de Quest no se mezclan paneles, patrones WebXR y escenarios contextuales: cada bloque conserva un único modelo de interacción. Una batería procedural WebXR puede incluir varios ejercicios y vueltas dentro de una sola inmersión persistente; los descansos, la pausa y el cambio de patrón no cierran la `XRSession`.
...
El `event_log` conserva por fase: ejercicio, vuelta, tipo, modo de dosis, dispositivo, tiempo activo, objetivo de repeticiones, cantidad informada, transiciones de colocación/retiro de VR Box, transferencia PC → Quest y resultado completo/parcial/omitido. En WebXR procedural agrega geometría, cobertura, parámetros angulares, cantidad de recentrados, avisos de desviación y pérdidas de sesión; nunca guarda poses crudas de cabeza. La finalización agrega los autorreportes y actualiza el estado en una transacción.
